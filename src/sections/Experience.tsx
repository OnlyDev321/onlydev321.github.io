    ...
    <Section id="experience" aria-labelledby="experience-heading">
      <Container>
        <SectionHeader
          ...
          title={[
            "Where the ",
            <span className="serif-italic">reps</span>,
            " came from.",
          ]}
          lede="Software engineering and academic research in Seoul, South Korea since 2023 — building scalable web platforms, real-world AI integrations, and cross-cultural engineering bridges."
        />
                    ...
                    <article className="rounded-[var(--r-md)] border border-[color:var(--line)] bg-[color:var(--surface)] p-5 shadow-[var(--shadow-sm)] sm:p-6">
                      <div className="flex flex-col gap-x-6 gap-y-2 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
                            <h3 className="font-display text-h3 text-[color:var(--ink)]">
                              {exp.role}
                            </h3>
                            {exp.current && (
                          ...
                          <p className="mt-1 text-body font-medium text-[color:var(--ink)]">
                            {exp.company}
                          </p>
                        </div>

                        <div className="flex flex-col gap-1 sm:shrink-0 sm:items-end sm:text-right">
                          <span className="data text-[color:var(--ink-muted)]">
                            {exp.period}
                          </span>
                      ...
                      <ul className="mt-4 space-y-2.5 border-t border-[color:var(--line)] pt-4">
                        {exp.bullets.map((bullet) => (
                          <li
                            key={bullet}
                            className="flex gap-3 text-small text-[color:var(--ink-muted)]"
                          >
                            <span
                              aria-hidden="true"
                  ...
                  <li key={`${edu.institution}-${edu.period}`}>
                    <Reveal delay={Math.min(i * 0.06, 0.18)}>
                      <article className="rounded-[var(--r-md)] border border-[color:var(--line)] bg-[color:var(--surface)] p-5 shadow-[var(--shadow-sm)]">
                        ...
                        <p className="mt-1 text-small text-[color:var(--ink-muted)]">
                          {edu.institution}
                        </p>
                        <p className="mt-2 data text-[color:var(--ink-faint)]">
                          {edu.period}
                        </p>
                        {edu.note && (
                          <p className="mt-2 text-small text-[color:var(--ink-muted)]">
                            {edu.note}
                          </p>
                        )}
                      </article>
